...
hdrAlgorithm::hdrAlgorithm(int n)
{
	m_multiContrast8.resize(n);
	m_multiContrast12.resize(n);
}
...
void hdrAlgorithm::getHdrImage(vector<Mat>&multiExposureImages_in, const string& path, const string& imageType, Mat& outputImage) {
	...
	m_multiExposureImages = multiExposureImages_in;
	int num = multiExposureImages_in.size();
...
Mat hdrAlgorithm::getContrastImage(const Mat& srcImage, int midValue) {
	...
	*/
	...
	kenel.at<float>(1, 1) = static_cast<float>(midValue);
...
void hdrAlgorithm::doFilter(vector<Mat>&inputImages, vector<Mat>&outputImages) {
	...
	for (size_t i = 0; i < n; i++) {
		...
		//cv::medianBlur(inputImages[i],outputImages[i],3);
	}
	...
	for (size_t i = 0; i < n; i++) {
		for (size_t r = 0; r < rows; r++) {
			for (size_t c = 0; c < cols; c++) {
				...
				outputImages[i].at<float>(r, c) = static_cast<float>(std::pow(2.7, mi));//高斯
			}
		}
